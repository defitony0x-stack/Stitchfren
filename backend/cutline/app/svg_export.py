"""
SVG rendering for pattern pieces and nested layouts.
Adapted from original v1 with minor cleanups for v2.
"""
...
def render_nested_layout_svg(
    piece_lookup: Dict[str, List],
    placements: List[Dict],
    fabric_width_cm: float,
    fabric_length_cm: float,
    scale: float = 4,
):
    from .nesting.engine import flip_points
    ...
    for pl in placements:
        ...
        pts = flip_points(base_points) if pl.get("rotated_180") else base_points
        xs = [p[0] for p in pts]
        min_x = min(xs)
        ...
        d = _piece_path(pts, offset_x=ox - min_x * scale, offset_y=oy_top + min_y * scale)
        piece_svgs.append(
