...
export const PRICE = process.env.STITCHFREN_PRICE || "$0.15";
...
app.get("/mcp/status/:taskId", async (req, res) => {
  try {
    ...
    res.json(result ? { status: "completed", result } : { status: "processing" });
  } catch (err) {
...
app.post("/mcp/draft-and-nest", async (req, res) => {
  ...
  try {
    ...
    return res.json({ ok: true, pending: true, task_id });
  } catch (err) {
